...
Start with `storytelling.md` for the reasoning, or `cli.md` if you need to run something.
...
Multiplying the axes gives the thirteen pipelines. `pipelines.md` develops them fully.
...
**The Validator governs the whole ladder.** "Could not" is defined in exactly one place — see `components.md` — which is what makes escalation a policy rather than a habit spread across three components.
