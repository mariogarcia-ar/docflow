...
| Status | `OCR-01`…`OCR-11` **DONE** - the contract is frozen, the Docling seam is in place, the pipeline/configuration primitives are implemented, a real conversion is extracted into the engine-independent `OCRDocument`, the blocks are normalized into one coordinate frame and ordered by a rule rather than by arrival, the three canonical representations are built with no run-time data in them, the detected tables are exported in reading order under zero-padded names, the content metrics are measured from a real conversion on both a content page and a blank one, the verdict classifies a result into a typed state without ever throwing, every artifact is published atomically with a `metadata.json` that carries the seven required keys, and one public entry point runs the twelve stages in order and returns a contract result for both outcomes — a success that published its whole namespace and a failure that published nothing; `OCR-12` … `OCR-14` `NOT_STARTED` |
...
- **Where `preserve_reading_order` lives is a divergence in the plan, and it is recorded rather
  ...
  order. It shipped in `layout.py` because it calls `normalize_bbox` for every item, so putting it in
  `rendering.py` would make the module that owns the coordinate frame depend on the module that owns
  Markdown for no reason, and `layout.py` already holds the ordering key. The plan-surface test lists
  it in the *Layout* group for the same reason. A first draft of this evidence block said the group
  had "five names, name for name"; it has **four**, and the fifth is a declared exception. The
  correction is recorded as divergence 10 in `README.md`.

- **This task's justification for fusing normalization and ordering was wrong, and `OCR-11`'s
  mutation battery proved it.** The original claim was that the two "would disagree the moment a page
  was not square". They cannot: normalization divides each axis by that axis' own size, so it is
  monotone per axis and preserves every ordering comparison. Measured on the committed fixture, the
  engine's pixel frame, the normalized frame and a 10×-scaled frame give the **identical** order, and
  what reorders a page is the **origin flip** (`BOTTOMLEFT` → `TOPLEFT`), not the scale. The pair is
  still one function, but for the reason the corrected docstring gives — a caller who had to remember
  both could forget one — and the false claim is struck from `layout.py`. It was found by a mutation
  that was *expected* to kill and survived, then measured rather than explained away.
...
**`publish_metadata` cannot appear in the timing record it writes**, and that is a property of the design rather than an oversight: the stage's own duration is unknowable at the moment the record containing it is assembled. `RECORDED_STAGES` names the stages that *can* be sealed plus `total` (twelve keys: eleven stages and the total), and the total is sealed inside `seal_timing` immediately before the payload is built — called from both the success path and `finish`, so a failed run reports its duration for the same reason a successful one does. A test asserts the artifact's `timing` keys are **exactly** `RECORDED_STAGES`, so a stage that stopped being recorded shows up as a failure rather than as a quietly partial account.

**The OCR-05 wiring is what this task exists to do, and it is observable in the artifact.** Before it, `normalize_layout` and `preserve_reading_order` appeared only in docstrings, so `document.json` carried the engine's pixel frame and the engine's iteration order while §9's frozen decision 4 required "normalized 0-1 coordinates". `OCR-06` recorded that gap as a test that would fail the moment the wiring landed, and it did. The wiring now runs in the `order` stage, and the evidence is positional rather than merely numerical: measured, the engine lists `table_001` **last** (index 19) while `preserve_reading_order` puts it at **index 10** — between the line items and the totals, where it is on the page — and the layout goes from `840 x 1036` to `1.0 x 1.0`. A test that only asserted "the order changed" would pass under either, since the engine's order is a perfectly plausible list.
...
1. **`metrics` as a method name was shadowed by `metrics` as a dataclass field.** The stage tuple held an `OCRMetrics` value where it expected a callable — the run died at the ninth stage with `TypeError: _Run.metrics() missing 1 required positional argument: 'self'`. The method is now `measure`, and the docstring says why it cannot be called `metrics`.
2. **`OCRError` is not an exception.** It is the contract's plain `@dataclass` with no `Exception` in its MRO, so the first draft's `except (OCRError, OCREngineError)` was dead code dressed as error handling — and it would have been the *only* handler for a contract failure, which instead travels by being returned. The handler catches `OCREngineError`, `OSError` and `ValueError`, and `_as_error` maps the engine's vocabulary to `ENGINE_ERROR` and everything else to `INTERNAL_ERROR`.
3. **`test_composition.py` broke `test_engine.py`'s lazy-seam invariant by existing.** Inside `primitives/`, `test_composition` sorts before `test_engine`, so the new module converted the fixture — and loaded Docling — before the seam's "importing the package loads no engine" assertions ran. Both failed in the full suite and passed in isolation, which is the signature of an ordering collision rather than a defect. The module was moved to `tests/ocr/test_entrypoints.py`, mirroring `src/docflow/ocr/entrypoints.py` exactly as the image and PDF suites mirror theirs, and the docstring records the collision so the name is not "tidied" back.
4. **The `_Run` record tripped `too-many-instance-attributes` (22/20)**, which the image processor's parallel record also does. Suppressed in place with the same stated reason, and seven docstring lines that exceeded pylint's 100-character limit were rewrapped.
5. **`publish_metadata` cannot appear in the timing record it writes**, which is why `RECORDED_STAGES` exists as a value distinct from `RUN_STAGES`. The stage's own duration is unknowable at the moment the record containing it is assembled, so the record names the stages that *can* be sealed plus `total` — twelve keys in all.

Mutation battery — **12 mutations, all killed**. Each was applied to `composition.py` or `entrypoints.py`, `__pycache__` purged and `PYTHONDONTWRITEBYTECODE=1` set before running `tests/ocr/test_entrypoints.py` (a restored source inside one filesystem-timestamp tick otherwise leaves the mutated `.pyc` and the mutation keeps running), then the source restored and the suite re-run green:

| # | Mutation | Killed by |
|---|---|---|
| M1 | `order`: drop `preserve_reading_order`, keep the engine's order | the table-index assertion (19 ≠ 10) |
| M2 | `order`: drop `normalize_layout`, keep engine pixels | the unit-square layout assertion |
| M3 | `order`: order against a rescaled (non-normalized) frame | the reading-order assertion |
| M4 | `validate_result`: treat `INCOMPLETE` as publishable | the direct stage test against an incomplete namespace |
| M5 | `finish`: do not abandon the namespace | the failed-run-publishes-nothing tests |
| M6 | `persist`: write nothing | the four-artifacts-present assertion |
| M7 | `measure`: record zeros instead of measuring | the metrics assertions |
| M8 | `_publish_metadata`: drop the processing key from the settled record | the verbatim-key assertion |
| M9 | `persist`: publish no per-table artifact | the `tables/table_001.md` assertion |
| M10 | `entrypoint`: delegate without the key | the verbatim-key assertion |
| M11 | `validate_request`: skip the request check | the missing-input failure tests |
| M12 | `validate_input`: skip the input check | the missing-input failure tests |

**The first run of the battery killed 8 of 12, and all four non-kills were findings rather than noise.** M3 survived because the production code was wrong, not the test — the rebuild described above; it is now the inverse mutation and it dies. M4 survived because **the guard was unreachable from the entry point**: `persist` writes exactly the four artifacts the validator requires, so `INCOMPLETE` cannot arise end to end, and a test now drives `validate_result` against a namespace with one artifact absent, which is the only place the refusal is observable. M8's anchor matched twice, because the processing key is recorded in both the provisional and the final metadata write — scoped to the final one. M12's anchor was stale after Ruff re-wrapped the stage. A second run closed M11 (also a stale anchor) and exposed **one equivalent mutant, which is now proved equivalent rather than merely reported**: `preserve_reading_order` computes the order on normalized geometry, so scaling both page dimensions by the same factor cannot change it — normalization divides each axis by that axis' own size, making it monotone per axis and therefore order-preserving. Measured, the engine's pixel frame, the normalized frame and a 10×-scaled frame give the **identical** order. That is a theorem, not a gap, and `test_the_reading_order_is_independent_of_the_sorting_frame` asserts it so a reader cannot mistake one for the other.

That last measurement **refuted a claim this processor had carried in writing for six tasks**: `layout.py` said normalizing and ordering are one step because "the two would disagree the moment a page was not square". They cannot disagree. What reorders a page is the **origin flip** (`BOTTOMLEFT` → `TOPLEFT`), which reverses one axis, not the scale. The docstring and the OCR-05 evidence are corrected, the pair is still one function but for a different reason (a caller who had to remember both could forget one), and the correction came from a mutation that survived where the battery predicted a kill.

Four QA gates: `pytest` **1290 passed**, `ruff check .` clean, `ruff format --check .` 140 files already formatted, `pylint src tests` **10.00/10**.
