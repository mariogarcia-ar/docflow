...
| Status | `OCR-01`…`OCR-11` **DONE** - the contract is frozen, the Docling seam is in place, the pipeline/configuration primitives are implemented, a real conversion is extracted into the engine-independent `OCRDocument`, the blocks are normalized into one coordinate frame and ordered by a rule rather than by arrival, the three canonical representations are built with no run-time data in them, the detected tables are exported in reading order under zero-padded names, the content metrics are measured from a real conversion on both a content page and a blank one, the verdict classifies a result into a typed state without ever throwing, every artifact is published atomically with a `metadata.json` that carries the seven required keys, and one public entry point runs the ten stages in order and returns a contract result for both outcomes — a success that published its whole namespace and a failure that published nothing; `OCR-12` … `OCR-14` `NOT_STARTED` |
...
**Status: DONE.** `src/docflow/ocr/primitives/composition.py` (the eleven-stage flow, `RUN_STAGES` and `RECORDED_STAGES`), `src/docflow/ocr/entrypoints.py` (the published surface, now a delegation rather than a stub), tests in `tests/ocr/test_entrypoints.py` — 27 tests. Measured against real conversions on both committed fixtures: the content page reaches `success` / `VALID` with 761 characters, 98 words, 19 blocks, 1 table and 15 paragraphs published under all four required names plus `tables/table_001.md`; the blank page reaches `success` / `EMPTY` with its namespace still populated. `process_ocr_from_page` was **not** shipped: §9's resolved decision 5 defers it to Phase 3, so the `# TODO: [MVP]` stub the task allowed was not needed either.
...
5. **`publish_metadata` cannot appear in the timing record it writes**, which is why `RECORDED_STAGES` exists as a value distinct from `RUN_STAGES`. The stage's own duration is unknowable at the moment the record containing it is assembled, so the record names the eleven stages that *can* be sealed plus `total`.
...
**The first run of the battery killed 8 of 12, and all four non-kills were findings rather than noise.** M3 survived because the production code was wrong, not the test — the rebuild described above; it is now the inverse mutation and it dies. M4 survived because **the guard was unreachable from the entry point**: `persist` writes exactly the four artifacts the validator requires, so `INCOMPLETE` cannot arise end to end, and a test now drives `validate_result` against a namespace with one artifact absent, which is the only place the refusal is observable. M8's anchor matched twice, because the processing key is recorded in both the provisional and the final metadata write — scoped to the final one. M12's anchor was stale after Ruff re-wrapped the stage. No mutation was withdrawn as equivalent, and `RECORDED_STAGES` needed no exception: the stage it omits is omitted for a reason the record itself proves.

Four QA gates: `pytest` **1286 passed**, `ruff check .` clean, `ruff format --check .` 140 files already formatted, `pylint src tests` **10.00/10**.
