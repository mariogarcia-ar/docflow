...
- **Out of bounds:** No edge-case matrix beyond the four scenarios; no real engines (the fakes stand in for the Phase 1 processors); no source change kept in place to satisfy a test; no config-wide rule suppression. These fakes are **contract-level** — they replace whole processors; the replay loaders of `pdf`, `image` and `ocr` (`README.md` §9.7) are **primitives-level** — they replace only the engine inside a real processor. Neither stands in for the other.
- **Acceptance criteria:**
