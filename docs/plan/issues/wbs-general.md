...
- **Scope / Deliverables:** a written reconciliation of `docs/plan/README.md` against `docs/idea/readme.md` and the five `docs/idea/procesador-*.md` files, listing every divergence and its resolution; the confirmation that the deliverable names in the code still match the idea's §"Naming"; explicit recording of the resolved naming drift between `consolidate_page`/`consolidate_document` (`README.md` §5) and `consolidate_page_result`/`consolidate_document_result` (`ORC-17`).
