...
- **Scope / Deliverables:** `tests/fakes/engines/fake_opencv.py` — an in-memory fake injected at the **`image/primitives/`** functions (`load_image`, the transformations), returning the engine's **native** output (small synthetic pixel arrays plus raw score values such as `blur`, `sharpness`, `contrast`) and able to raise `DECODE_ERROR` so the failure path of subplan §6 is reachable. The fixture patches the module attribute the seam resolves at call time (`docflow.image.primitives.cv2`) with a plain namespace exposing only the attributes the seam uses, so no test imports `cv2` and importing the seam needs no OpenCV.
