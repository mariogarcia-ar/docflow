...
The only test double for Poppler is an **in-memory fake injected at the engine call**
(`README.md` §9.7). We do not test Poppler: no test invokes, imports or asserts it. Poppler
is reached through CLI subprocesses and hands back **files plus an exit code and stderr — no
return value**, so this fake is shaped by the subprocess contract:
...
function call, so it **cannot share a helper** with the OCR/Image doubles; and a
session-scoped fixture buys nothing here, because `dpi` is not a lever on the fake — it is a
per-test option.
...
- **Wave 2 (primitives, parallel):** PDF-03, PDF-04, PDF-05, PDF-06, PDF-07, PDF-08 — independent once the primitives skeleton exists — plus PDF-14 (the Poppler double, which lets the whole processor be tested with no engine installed).
...
Every test of this module — the happy path, the three invariants, the classification
vectors, the validation states and the atomic-publication failure path — runs on the
in-memory Poppler double (placement and injection point in §3) and never reaches Poppler.
The suite must pass with no engine installed; there is no real tier, no marker and no skip
rule (`README.md` §7, §9.7).

### Happy-path test
The test drives `process_pdf` over a small committed multi-page fixture with the engine call
doubled, and asserts a `PDFResult` with `status == SUCCESS`, `len(pages) == page_count`,
every page carrying non-empty `page.pdf`, `page.png`, `text.txt`,
...
### Failure fixtures, in two buckets

Every existing failure fixture is classified explicitly, because the bucket decides the test
mechanism:

| Fixture | Bucket | Test mechanism |
|---|---|---|
| `pdf_corrupt.pdf` | **Pre-engine** — *if* `validate_pdf`'s fail-fast returns `CORRUPTED_PDF` before Poppler is invoked. Confirm this at `PDF-11` start: if the subprocess is reached first, the case moves to *Injected* and the double returns the failing exit code | an ordinary unit test: no live call |

Engine breakage that no input reproduces (a subprocess that raises, a timeout) is
**injected** through the double, never simulated by feeding it a bad file.
`ENCRYPTED_PDF` / `UNSUPPORTED_PDF` are typed fail-fast paths of `validate_pdf` and are
...
- The Poppler double of §3 (`tests/fakes/engines/fake_poppler.py`, native artifacts plus exit
  code, injected at the `subprocess.run` call) is agreed; `PDF-14` is a row in §4.
...
- The **no-engine rule** holds: the whole suite passes with Poppler absent, no test invokes, imports or asserts the engine, and the double is never reachable as a fallback (nothing under `src/docflow/` imports `tests/`).
...
|---|---|---|
| Engine/library drift (Poppler versions) | Silent output differences | Pin versions; surface `engine` + `engine_version` in every artifact's `metadata.json`; deterministic ordering. The engine itself is not tested (`README.md` §9.7); the drift is caught on the pin bump, not by a test. |
