...
| **E03** | Slot & resource policy for the corpus | `S3-T11` | 1 | Bound the corpus run's CPU and GPU work before the run exists, so per-stage timings stay meaningful. | **2 — Golden set / tests** (`S3-T11` — *"GPU work is serialized and the model is loaded once"*; §13 Track 1 rung 3) — the `--jobs` surface behind it is `S3-T12`'s and is labelled on E02 |
...
| **E06** | Invalidation & corpus-scale resume | `S3-T08`, `S3-T09` | 2 | Make a forced reprocess invalidate everything downstream, and make an 11k-file run resumable at the exact stage of the in-flight document. | **2 — Golden set / tests** (`S3-T09` — *"Resume skips completed work"*) · **4 — CLI (to probe)** (`S3-T08`) |
...
| **E08** | The Stage 3 gate (corpus) | `S3-T14` | 1 | Close Stage 3 and the PoC: 11k files, one command, interrupted and resumed, with the baseline recorded. | **1 — Fast flow** (`S3-T14`) · **2 — Golden set / tests** (`S3-T14` — *"The baseline is recorded, not targeted"*) |
...
Lane coverage note, stated rather than papered over: `plan-03-pipelines.md` §13's four **summary** rows name a task range per track, and three tasks do not appear in those summary cells by name — `S3-T09`, `S3-T10` and `S3-T11`. Each of the three **is** named in that track's own sub-table beneath the summary: `S3-T09` in Track 1's rung 3 and in Track 2's *"Resume skips completed work"* row; `S3-T10` in Track 2's *"The manifest is derived"* row; `S3-T11` in Track 1's rung 3 and in Track 2's *"GPU work is serialized and the model is loaded once"* row. (`S3-T03` is likewise absent from the summary cells but is covered by Track 3's range `S3-T01`–`S3-T04`.) **No task in this plan is unnamed by §13.** The lane column above records the tracks §13 supports, and no lane is invented for a task §13 does not name — which is why E03 cites Track 2 and not Track 4: the `--jobs`/`DOCFLOW_JOBS` surface row is `S3-T12`'s, and it is labelled on E02 where it belongs.
...
**A stage is `done` only when its flow closes, and this directory does not restate that — it inherits it.** Ticking all 14 issues does **not** close the PoC: `E08-01` (`S3-T14`) is the gate, and it closes when 11k documents run in one command over a mirrored tree, **interrupted mid-run and resumed**, with the baseline recorded (`plans/README.md` §2, `plan-03-pipelines.md` §3, §11). Two ordering facts a reader needs before declaring anything closed: the gate's five predecessors span **four distinct epics** (§3), and a green `S3-T13` is a **precondition** of `S3-T14`, not a sibling — the shape contract is verified before the corpus run, not alongside it (`epic-07-shape-identity.md` §1).
