...
- [ ] **No CLI flag exists** for any of the five, and a search asserts the absence of a `--min-dpi`-shaped flag on any surface.
...
- **A `--min-dpi`, `--min-chars`, `--cut-confidence`, `--correct` or `--tolerance-amounts` flag.** **Never** (ADR-009, `prd.md` NFR-06a).
