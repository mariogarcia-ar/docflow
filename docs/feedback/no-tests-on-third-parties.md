...
| `issues/wbs-procesador-{pdf,image,ocr}.md` §1 | a **Test framing** paragraph: the assertions of every primitive are about our translation, naming, ordering and error mapping, never about what the engine returns |
| `issues/wbs-procesador-image.md` IMG-03, IMG-04, IMG-05, IMG-08 | task criteria that asserted **engine readings** re-written: "dimensions match the file" → the refs carry the seam's values; "a non-zero skew angle is reported" → the seam's reading is surfaced; "the output has a single channel" → the artifact goes to a distinct path and the source is untouched; "the VLM variant preserves colour" → each variant is written by its own pipeline and its transformations are recorded. The numeric correctness of `calculate_*_score` is now named as **not covered**, deliberately |
| `issues/wbs-procesador-pdf.md` PDF-02, PDF-03, PDF-04, PDF-06, PDF-07 | same re-framing: a missing engine binary is no longer a probe ("the double's failing exit code"), and page count, page order, text extraction and image naming are asserted as **our** loop and naming, not Poppler's output |
